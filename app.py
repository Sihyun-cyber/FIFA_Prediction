...
print("--- [시작] 예측 서버를 위한 초기 설정을 시작합니다. ---")

app = Flask(__name__)

# 1단계에서 저장한 결과물들 로드
try:
    model = tf.keras.models.load_model('player_aging_curve_model.h5')
...
def predict_player_form_change(player_name, max_target_year=2030):
    ...
    for i in range(1, max_target_year - current_year + 1):
        ...
        predicted_change = model.predict(predict_df_scaled[final_features], verbose=0)[0][0]
...
def predict_api():
    ...
    # --- ⭐ [그래프 생성] 축구 테마 스타일링 적용 시작 ⭐ ---
    sim_df = pd.DataFrame(sim_data)
    
    # 1. figure와 axes 객체를 생성하여 세부 스타일링 준비
    fig, ax = plt.subplots(figsize=(12, 6))

    # 2. 배경색 설정 (바깥쪽, 안쪽)
    fig.patch.set_facecolor('#0d1f0c') # 짙은 잔디색
    ax.set_facecolor('#133311') # 살짝 밝은 잔디색

    # 3. 데이터 분리 및 그래프 그리기 (색상 및 마커 변경)
    ...
    plt.close(fig) # figure 객체를 명시적으로 닫아주기
